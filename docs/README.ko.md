...
2. devrig 자체의 템플릿 파일(브랜딩 에셋, 번역된 문서, `CITATION.cff`, `CONTRIBUTING.md`, `LICENSE`)을 **제거**해 워크스페이스가 깔끔하게 시작되도록 합니다 — devrig가 아니라 당신의 프로젝트니까요.
3. 그곳에 새로운 git 저장소를 **초기화합니다**.
4. devrig의 대화식 설정 마법사를 즉시 **실행합니다** — 프로젝트 이름, 설명, GitHub org, 저장소, 이슈 트래커, 기능 토글을 같은 명령어 안에서 설정합니다. 마법사는 devrig의 README 대신 *당신의* 프로젝트를 위한 `README.md`도 생성합니다.
...
devrig의 설정 로직을 다시 구현하지 않습니다 — 템플릿을 가져와서 템플릿 전용 파일을 제거한 뒤 devrig 자체의 `setup.sh`에 넘길 뿐입니다(그쪽에서도 제거하므로 GitHub의 "Use this template" 경로도 동일한 결과가 됩니다). devrig의 설정 흐름, 스킬, 설정 스키마가 업데이트되면 다음에 누군가 `npx create-devrig`를 실행할 때 자동으로 반영됩니다. 이 패키지 자체는 거의 변경할 필요가 없습니다.
